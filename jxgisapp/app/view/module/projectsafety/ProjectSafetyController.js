 ...
 */
Ext.define('jxgisapp.view.module.projectsafety.ProjectSafetyController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.projectsafety',
    ...
    moduleInit: function (me) {
        me.queryProjSafetyData();
    },
    //查询数据
    queryProjSafetyData: function () {
        var me = this;
        //加载统计信息
        var meView = this.getView();
        var st = meView.lookupReference('queryProjSafetyStartDate').getRawValue();
        var et = meView.lookupReference('queryProjSafetyEndDate').getRawValue();
        var keywords = Ext.getCmp('projSafetyKeyWordId').getValue();

        var gridCom = Ext.getCmp('projSafetyGrid');

        var store = gridCom.getStore();
        store.proxy.url = conf.rtmdataUrl + 'rtmdata';//TODO 2018-04-23---本地数据加载暂时屏蔽，若需要加载后台服务数据，需要解除注释
        // store.proxy.url = 'resources/json/projectsafety.json';
        store.load({
            params: {
                st: st,
                et: et,
            ...
            callback: function (records, options, success) {
                if (success) {
                    ...
                    me.loadProjSafetyLayer(records);
                }
            },
            scope: store,
            add: false
    ...
    //加载图层数据
    loadProjSafetyLayer: function (features) {
        require(
            [
